        ...
        <div class="cart__block">
          ...
          <Transition name="fade" mode="out-in" :duration="{ delay: 500 }">
            <div v-if="totalOffersQuantity" class="cart__total">
              <p>
                Количество товаров: <b>{{ totalOffersQuantity }}</b> шт.
              </p>
              <p>
                Сумма: <b>{{ $filters.numberFormat(totalPrice) }} ₽</b>
              </p>
            </div>
          </Transition>
...
export default {
  ...
  methods: {
    ...
    deliveryChanged(value) {
      ...
      axios
        .get(`${API_BASE_URL}/api/payments`, { params: { deliveryTypeId: value } })
        .then((response) => {

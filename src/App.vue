...
export default {
  ...
  setup() {
    ...
    const isSomeOpen = useModal().isSomeOpen;
    ...
    const updateUserAccessKey = (key) => {
      store.commit("updateUserAccessKey", key);
    };

    // Load cart and update user access key on component mount
    onMounted(() => {
      if (userAccessKey) {
